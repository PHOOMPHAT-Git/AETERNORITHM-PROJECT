...
router.post('/character', verifyGameSecret, async (req, res) => {
    try {
        ...
        await GameCharacter.findOneAndUpdate(
            { roblox_user_id, slot },
            { $set: update },
        ...
        console.error('Error saving game character:', error);
...
router.post('/character/batch', verifyGameSecret, async (req, res) => {
    try {
        ...
        console.error('Error batch saving game characters:', error);
        res.status(500).json({ success: false, error: 'Internal server error' });
    }
});
...
router.post('/player-left', verifyGameSecret, async (req, res) => {
    try {
        ...
        await GameCharacter.updateMany(
            { roblox_user_id },
