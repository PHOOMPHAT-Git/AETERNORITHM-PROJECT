...
router.get('/callback', async (req, res) => {
    try {
        ...
        await getRobloxVerifyModel().findOneAndUpdate(
        ...
        // Update roblox_user_id in User model (if discord account is linked)
...
router.get('/pending-roles', async (req, res) => {
    try {
        ...
        const pendingUsers = await getRobloxVerifyModel().find({
            status: 'verified',
...
router.get('/start/:discordUserId/:guildId', async (req, res) => {
    try {
        ...
        const existingVerify = await getRobloxVerifyModel().findOne({
            discord_user_id: discordUserId,
            status: 'verified'
        });
